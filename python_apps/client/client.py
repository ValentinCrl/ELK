...
class JsonTraceFormatter(TraceInjectingFormatter):
    def format(self, record):
        super().format(record)
        return json.dumps({
            ...
            "service": os.getenv("SERVICE_NAME", "api-client"),
            "trace_id": record.trace_id,
            "span_id": record.span_id,
            "message": record.getMessage(),
        })

log_formatter = JsonTraceFormatter()
console_handler = logging.StreamHandler(sys.stdout)
console_handler.setFormatter(log_formatter)

log_file_path = os.getenv("LOG_FILE", "/var/log/apps/client.log")
